...
gen.xy.err <- function(x, y, mean.fn=mean, var.fn=sderr, ...) {
	xe <- gen.err(x, mean.fn, var.fn, ...)
	ye <- gen.err(y, mean.fn, var.fn, ...)
	res <- data.frame(xe, ye)
	colnames(res) <- c('x','x.lower', 'x.upper', 'y', 'y.lower', 'y.upper')
	res
}
...
plot.err <- function(x, y=NULL, x.lower=NULL, x.upper=NULL, y.lower=NULL, y.upper=NULL, add=FALSE, log='', col='black', bar.col=NULL, type='p', ...) {
	...
	if (type=='l' | type=='o') {
    	...
    	points(x, y, type=type, col=col, ...)
    }
}

points.err <- function(x, y=NULL, x.lower=NULL, x.upper=NULL, y.lower=NULL, y.upper=NULL, bar.col=NULL, ...) {
	if (is.null(y)) {
		# Assume this is a matrix/dataframe generated by gen.err
		plot.err(x$x, x$y, x$x.lower, x$x.upper, x$y.lower, x$y.upper, add=TRUE, bar.col=bar.col, ...)
	} else {
		plot.err(x,y,x.lower,x.upper,y.lower,y.upper,add=TRUE, bar.col=bar.col, ...)
	}
}

lines.err <- function(x, y=NULL, x.lower=NULL, x.upper=NULL, y.lower=NULL, y.upper=NULL, bar.col=NULL, type='l', pch=NULL, ...) {
	if (is.null(y)) {
		# Assume this is a matrix/dataframe generated by gen.err
		plot.err(x$x, x$y, x$x.lower, x$x.upper, x$y.lower, x$y.upper, add=TRUE, bar.col=bar.col, type=type, ...)
	} else {
		plot.err(x,y,x.lower,x.upper,y.lower,y.upper,add=TRUE, bar.col=bar.col, type=type, ...)
	}
	if (!is.null(pch)) {
		points(x,y, pch=pch,...)
	}
...
labline <- function(g, method, x=NULL, length.out=100, log.x=TRUE, slope=NULL, ...) {
	...
	# Get coefficients into same format: c(intercept, slope)
	if (class(g)=='lm') {
		coefs <- coef(g)[1:2]
	} else if (class(g)=='lmodel2') {
...
multi.ecdf <- function(x, log=F, col=NULL, lty="solid", lwd=1, 
	...
	if (!is.null(legend.at)) {
		...
		legend(legend.at[1], legend.at[2], cex=legend.cex, col=legend.cols, legend=legend.names, lty=ltys, bty=legend.bty)
	}
...
multi.density <- function(x, log=FALSE, type='l', kernel="rectangular", bw='SJ', col=NULL, lty="solid", 
	...
	if (!(type=='n')){
		...
		if (!is.null(legend.at)) {
			...
			legend.cols <- col[1:min(length(x), length(col))]
			if (fill) {
				legend(legend.at[1], legend.at[2], legend=legend.names, fill=legend.cols, cex=legend.cex, bty=legend.bty, lwd=lwds)
			}
			else {
