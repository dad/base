pcor.test <- function(x,y,z,use="mat",method="s",na.rm=T){
	...
	res <- list(estimate=pcor,p.value=p.value,statistic=statistic,n=n,gn=gn,method=p.method,use=p.use)
    class(res) <- "pcor"
    return(res)
}

print.pcor <- function(x) {
  print(data.frame(x[c("estimate","p.value","statistic","n","gn","method","use")]))
}
