...
def main(args):
	...
	# Print it
	(head, fname) = os.path.split(pdb_file)
	output_file.write('>%s-%s Residue sequence for chain(s) %s from PDB file %s\n%s' % \
		(fname.split(".")[0], (''.join(chain_identifiers)).strip(), chain_identifiers, pdb_file, residue_seq))
